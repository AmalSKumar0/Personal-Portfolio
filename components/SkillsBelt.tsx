...
export const SkillsBelt: React.FC = () => {
  return (
    <div className="w-full bg-gradient-to-br from-cream via-white to-cream py-16 flex flex-col justify-center overflow-hidden relative border-y border-neon-blue/10">
      ...
      {/* Left Fade Gradient */}
      <div className="absolute inset-y-0 left-0 w-32 bg-gradient-to-r from-cream via-cream/80 to-transparent z-10 pointer-events-none" />
      
      {/* Right Fade Gradient */}
      <div className="absolute inset-y-0 right-0 w-32 bg-gradient-to-l from-cream via-cream/80 to-transparent z-10 pointer-events-none" />
        ...
        <div className="flex animate-infinite-scroll group-hover:paused">
          {skills.map((skill, index) => (
            <div key={`list-1-${index}`} className="flex items-center gap-3 mx-8 select-none group/item">
              <div className="p-3 bg-white rounded-xl border-2 border-neon-blue/20 text-neon-blue shadow-lg hover:shadow-neon-blue/20 transition-all duration-300 hover:scale-110 hover:border-neon-blue/40">
                <skill.icon size={28} className="animate-icon-spin" />
              </div>
              <span className="text-tech-dark font-mono font-semibold whitespace-nowrap text-sm tracking-wide group-hover/item:text-neon-blue transition-colors duration-300">
                {skill.name}
              </span>
            </div>
          ))}
